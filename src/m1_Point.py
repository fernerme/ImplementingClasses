...
class Point(object):
    """ This class contructs a point at a specified location."""
    ...
    def move_to(self, x, y):
        distance_traveled = ((self.x-x)**2 + (self.y-y)**2)**0.5
        self.x = x
        self.y = y
        self.num_of_moves = self.num_of_moves + 1
        self.total_distance_traveled = self.total_distance_traveled + distance_traveled
    ...
    def get_distance_from_start(self):
        return ((self.x-self.original_x)**2 + (self.y-self.original_y)**2)**0.5

    def get_distance_traveled(self):
        return self.total_distance_traveled
    
    def closer_to(self, p2, p3):
        dist_to_p2 = ((self.x-p2.x)**2 + (self.y-p2.y)**2)**0.5
        dist_to_p3 = ((self.x-p3.x)**2 + (self.y-p3.y)**2)**0.5
        if dist_to_p2 < dist_to_p3:
            return p2
        elif dist_to_p2 > dist_to_p3:
            return p3
        else:
            return p2
